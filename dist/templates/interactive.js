 ...
 */
import { makeText, makeSeparator, makeButton, makeBadge, makeHorizontalStack, wrap, addTheme, makeInput, makeBarChart, makeProgress } from "./engine.js";
// ---------------------------------------------------------------------------
// Template: Poll
//
...
// ---------------------------------------------------------------------------
export function pollResults(question, options, counts, userVote, theme) {
    const elements = [];
    const kids = [];
...
export function quiz(slots) {
    ...
    pages.push({
        ...
        meta: {
            ...
            config: { total, baseId: slots.baseId },
        },
    });
    return { pages };
}
...
export function tokenBuy(slots) {
    ...
    // Buy button
    const params = { buyToken: slots.buyToken };
    if (slots.sellToken)
        params.sellToken = slots.sellToken;
    elements.push({
...
// ---------------------------------------------------------------------------
export function ratingResults(subject, avg, count, userRating, min, max, theme) {
    const elements = [];
    const kids = [];
...
// Template: Claimed (returned after claiming)
// ---------------------------------------------------------------------------
export function claimed(title, description, claimCount, theme) {
    const elements = [];
    const kids = [];
...
export function textEntry(slots) {
    ...
    kids.push("sep1");
    // Input field
    const inputProps = { name: inputName, label: slots.prompt };
    if (slots.inputType)
        inputProps.type = slots.inputType;
    if (slots.placeholder)
        inputProps.placeholder = slots.placeholder;
    if (slots.maxLength)
        inputProps.maxLength = slots.maxLength;
...
// ---------------------------------------------------------------------------
export function textEntryResults(prompt, userResponse, submissionCount, theme) {
    const elements = [];
    const kids = [];
