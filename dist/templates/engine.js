...
// Shared helpers
// ---------------------------------------------------------------------------
export function makeText(id, content, extra = {}) {
    return { type: "text", id, props: { content, ...extra } };
}
export function makeSeparator(id) {
    return { type: "separator", id, props: {} };
}
export function makeButton(id, label, action, params, variant = "primary") {
    return {
        type: "button", id,
        props: { label, variant },
...
export function makeBadge(id, label) {
    return { type: "badge", id, props: { label } };
}
export function makeHorizontalStack(id, children, gap = "sm") {
    return { type: "stack", id, props: { direction: "horizontal", gap }, children };
}
export function makeInput(id, name, extra = {}) {
    return { type: "input", id, props: { name, ...extra } };
}
export function makeSwitch(id, name, label, defaultChecked = false) {
    return { type: "switch", id, props: { name, label, defaultChecked } };
}
export function makeBarChart(id, bars, extra = {}) {
    return { type: "bar_chart", id, props: { bars: bars.slice(0, 6), ...extra } };
}
export function makeProgress(id, value, max, label) {
    const props = { value, max };
    if (label)
        props.label = label;
    return { type: "progress", id, props };
}
export function makeIcon(id, name, extra = {}) {
 ...
 */
export function wrap(allElements) {
    // Find root — should be the last element pushed
    const rootEl = allElements.find((e) => e.id === "root");
    if (!rootEl)
...
export function addTheme(snap, theme, effects) {
    if (theme)
        snap.theme = { accent: theme };
    if (effects?.length)
 ...
 */
export function addBadges(elements, labels, containerId) {
    if (!labels.length)
        return null;
...
function tutorial(slots) {
    ...
    if (total === 0)
        throw new Error("Tutorial needs at least 1 page");
    if (total === 1) {
    ...
    return slots.pages.map((page, idx) => {
        ...
        if (idx > 0 && remainingSlots >= 2) {
            const backBtn = makeButton("back", "← Back", "submit", { target: `${baseUrl}/${slots.baseId}-${idx - 1}` }, "secondary");
...
export function getSlotSchema(template) {
    const schemas = {
        explainer: {
