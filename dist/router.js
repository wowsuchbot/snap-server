 ...
 */
import { getSnapMeta, castVote, getVoteCounts, hasVoted, submitForm, claimSnap, getClaimCount, checkRateLimit, castRating, getUserRating, getSubmissionCount } from "./handlers.js";
 ...
 */
export async function handleSnapPost(snapId, fid, inputs, buttonIndex) {
    ...
    switch (meta.template) {
        case "poll":
            return handlePoll(snapId, fid, inputs, meta.config);
        case "quiz":
            return handleQuiz(snapId, fid, inputs, meta.config);
        case "quiz-results":
            return handleQuizResults(snapId, fid, meta.config);
        case "claim":
            return handleClaim(snapId, fid, inputs, meta.config);
        case "rating":
            return handleRating(snapId, fid, inputs, meta.config);
        case "text-entry":
            return handleTextEntry(snapId, fid, inputs, meta.config);
        case "form":
            return handleForm(snapId, fid, inputs, meta.config);
        default:
            return null;
    }
}
...
// Poll handler
// ---------------------------------------------------------------------------
function handlePoll(snapId, fid, inputs, config) {
    ...
    snap ? getFirstTextContent(snap.json) || "Poll Results" : "Poll Results", config.options, counts, userVote, theme);
    return { snapJson: resultsJson };
}
// ---------------------------------------------------------------------------
// Quiz handler
// ---------------------------------------------------------------------------
function handleQuiz(snapId, fid, inputs, config) {
    ...
    return null; // Let default handler re-serve the snap with updated state
}
function handleQuizResults(snapId, fid, config) {
    const state = getSnapState(snapId, fid);
    ...
    const snapJson = {
        version: "2.0",
        theme: { accent: "purple" },
        ui: { root: "root", elements: Object.fromEntries(elements.map((e) => { const { id, ...rest } = e; return [id, rest]; })) },
    };
    return { snapJson };
}
// ---------------------------------------------------------------------------
// Claim handler
// ---------------------------------------------------------------------------
function handleClaim(snapId, fid, _inputs, config) {
...
// Rating handler
// ---------------------------------------------------------------------------
function handleRating(snapId, fid, inputs, config) {
    ...
    const resultsJson = ratingResults(config.subject, result.avg, result.count, userRating, config.min, config.max, theme);
    return { snapJson: resultsJson };
}
// ---------------------------------------------------------------------------
// Text Entry handler
// ---------------------------------------------------------------------------
function handleTextEntry(snapId, fid, inputs, config) {
...
// ---------------------------------------------------------------------------
function handleForm(snapId, fid, inputs, _config) {
...
// Helpers
// ---------------------------------------------------------------------------
function makeErrorSnap(message) {
    const elements = [];
    const kids = [];
...
function getFirstTextContent(json) {
    try {
        const snap = JSON.parse(json);
        const textEl = Object.values(snap.ui?.elements || {}).find((e) => e.type === "text");
        return textEl?.props?.content || null;
    }
    catch {
        return null;
    }
}
function getSecondTextContent(json) {
    try {
        const snap = JSON.parse(json);
        const textEls = Object.values(snap.ui?.elements || {}).filter((e) => e.type === "text");
        return textEls.length > 1 ? textEls[1]?.props?.content || null : null;
    }
    catch {
        return null;
    }
