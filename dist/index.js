...
import { getSnap, upsertSnap, setSnapStateBulk, getDb, listSnaps, deleteSnap } from "./db.js";
import { validateSnapResponse } from "@farcaster/snap";
import { ensureMigrated, setSnapMeta, deleteSnapMeta } from "./handlers.js";
import { expand, listTemplates, getSlotSchema } from "./templates/engine.js";
...
app.use("/api/status/*", async (c, next) => {
    const db = getDb();
    const count = db.prepare("SELECT COUNT(*) as n FROM snaps").get().n;
...
app.get("/health", (c) => {
    const db = getDb();
    const count = db.prepare("SELECT COUNT(*) as n FROM snaps").get();
 ...
 */
async function deployTemplate(templateName, slots) {
 ...
 */
async function smokeTestInteractive(snapId, inputs) {
    ...
    try {
        ...
        throw new Error(`Smoke test FAILED for ${snapId}: ${err.message}. ` +
            `Snap was NOT deployed. Fix the handler and try again.`);
    }
}
async function deployInteractiveTemplate(templateName, slots) {
    const deployed = [];
    if (templateName === "poll") {
        const { snapJson, meta } = poll(slots);
        ...
        const { pages: quizPages } = quiz(slots);
        ...
        // Client-only — no server handler, no smoke test
        const { snapJson } = tokenShowcase(slots);
        ...
        const { snapJson, meta } = rating(slots);
        ...
        const { snapJson, meta } = textEntry(slots);
...
app.get("/:id", (c) => {
    ...
    const description = snapData.ui?.elements ? Object.values(snapData.ui.elements)
        .filter((e) => e.type === "text" && e.props?.weight !== "bold")
        .map((e) => e.props?.content)
...
app.post("/:id", async (c) => {
    ...
    const snap = getSnap(id);
    if (!snap)
        return c.json({ error: "Snap not found" }, 404);
    let body;
    try {
        body = await c.req.json();
    }
    ...
    if (skipVerify) {
        ...
        try {
            ...
            fid = decode(body).payload?.fid ?? null;
        }
        catch (err) {
