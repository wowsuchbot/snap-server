...
import { expand, listTemplates, getSlotSchema, type TemplateName } from "./engine.js";
...
if (command === "schema") {
  const template = args[1] as TemplateName;
  if (!template) { console.error("Missing template name"); process.exit(1); }
...
// Expand template
const template = command as TemplateName;
